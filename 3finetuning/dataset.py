...
class llavadataset(Dataset):
  def __init__(self, qa_dataset, phi_model_name, clip_model_name,processor):
...
def collate_fn(batch):
    ...
    ques_padded = torch.nn.utils.rnn.pad_sequence(ques, batch_first=True, padding_value=50296)
    ans_padded = torch.nn.utils.rnn.pad_sequence(ans, batch_first=True, padding_value=50296)
